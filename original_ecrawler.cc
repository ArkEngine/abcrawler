...
connect_context_t* connect_context_new( const char* host, event_base* base, evdns_base* dnsbase)
{
    TrueOrDie(NULL != host);
    ...
    my_connection_context->base = base;
    my_connection_context->dnsbase = dnsbase;
...
void request_cb(struct evhttp_request * my_evhttp_request, void * arg)
{
    ...
    {
//        printf("%s +%u my_evhttp_request = %p my_evhttp_connection: %p ++++\n",
//                __func__, __LINE__,my_evhttp_request, evhttp_request_get_connection(my_evhttp_request));
        struct evbuffer* buff = evhttp_request_get_input_buffer(my_evhttp_request);
        TrueOrDie(NULL != buff);
        size_t len = evbuffer_get_length(buff);
        my_connection_context->content = (char*)malloc(len+1);
        TrueOrDie(NULL != my_connection_context->content);
        my_connection_context->total_size = len;
        int read_len = evbuffer_remove( buff, my_connection_context->content, len);
        printf("read %d byte, expect %d bytes\n", read_len, len);
        TrueOrDie(read_len == len);
        my_connection_context->content[len] = 0;
        ...
        TrueOrDie (-1 != fd);
        TrueOrDie (my_connection_context->total_size 
                == write(fd, my_connection_context->content, my_connection_context->total_size));
        close(fd);
        printf("%u write into %s\n", request_count, DUMP_FILE_NAME);
        printf("--------------------\n");

        printf("Status-Code: %d\n", evhttp_request_get_response_code(my_evhttp_request));
        if (302  == evhttp_request_get_response_code(my_evhttp_request))
        {
            const char* new_location = evhttp_find_header(head_list, "Location");
            if (new_location)
            {
                printf("redirect to %s\n", new_location);
                request_count++;
                page_crawler(my_connection_context->base, my_connection_context->dnsbase, new_location);
            }
            else
            {
                printf("these is no Location infomation.\n");
            }
        }
    ...
    // TODO why conn = 0x00?
    // struct event_base* base = evhttp_connection_get_base(evhttp_request_get_connection(my_evhttp_request));
    // evhttp_connection_free (evhttp_request_get_connection(my_evhttp_request));
    //printf("ev_request: %p ev_conn: %p\n", my_evhttp_request, evhttp_request_get_connection(my_evhttp_request));
...
event_cb(struct bufferevent *bev, short events, void *ctx)
{
    ...
    static void
conn_close_cb(struct evhttp_connection *evconn, void *arg)
{
    printf("connection close for url[%s]\n", (char*)arg);
}
...
//    struct evhttp_connection* my_evhttp_connection = evhttp_connection_base_new (base, dnsbase, "222.73.6.36", port);
    TrueOrDie(NULL != my_evhttp_connection);
    evhttp_connection_set_closecb(my_evhttp_connection, conn_close_cb, strdup(url.c_str()));
//    printf("my_evhttp_connection: %p\n", my_evhttp_connection);
    ...
    //struct timeval tv;
    //tv.tv_sec  = TIMEOUT_IN_SECONDS * 3;
    //tv.tv_usec = 0; 
    ...
    connect_context_t* my_connection_context = connect_context_new( host.c_str(), base, dnsbase);
    ...
    int ret = evhttp_make_request (my_evhttp_connection, my_evhttp_request, EVHTTP_REQ_GET, path.c_str());

//    printf("%s +%u my_evhttp_request = %p my_evhttp_connection: %p ----\n",
//            __func__, __LINE__,my_evhttp_request, my_evhttp_connection);
//    printf("%s +%u my_evhttp_request = %p my_evhttp_connection: %p ++++\n",
//            __func__, __LINE__,my_evhttp_request, evhttp_request_get_connection(my_evhttp_request));
    return ret;
}
