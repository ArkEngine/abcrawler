...
bitmap :: bitmap (const char* dir, const char* file, const uint32_t filesize, bool memlock/* = true */)
{
    m_filesize = filesize;
    ...
    MySuicideAssert (-1 != lseek(m_fd, filesize, SEEK_SET));
    char tmpchar;
    ...
    if (memlock)
    {
        MySuicideAssert (MAP_FAILED != (puint = (uint32_t*)mmap(0, m_filesize,
                        PROT_READ | PROT_WRITE, MAP_SHARED|MAP_LOCKED, m_fd, 0)));
    }
    else
    {
        MySuicideAssert (MAP_FAILED != (puint = (uint32_t*)mmap(0, m_filesize,
    ...
    ROUTN("file[%s] filesize[%u] memlock[%u]", filename,  filesize, memlock);
}
